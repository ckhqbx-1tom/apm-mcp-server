...
async def get_alarms(
    ...
    params = {
        "view": "Extended", "severity": normalized_severity.title() if normalized_severity else None,
...
async def get_alarm_details(
    ...
    result = await get_alarms(client, resource_id=resource_id, page_size=MAX_PAGE_SIZE)
    matches = result["alarms"]
    if alarm_id is not None:
        matches = [item for item in matches if item["alarm_id"] is not None and str(item["alarm_id"]) == alarm_id]
    if resource_id is not None:
        matches = [item for item in matches if item["resource_id"] is not None and str(item["resource_id"]) == resource_id]
    if attribute_id is not None:
        matches = [item for item in matches if item["attribute_id"] is not None and str(item["attribute_id"]) == attribute_id]
    if created_at is not None:
        matches = [item for item in matches if item["created_at"] == created_at]
    if not matches:
